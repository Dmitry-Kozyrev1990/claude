...
**Note:** This skill uses the TubeLab API, not Apify — `APIFY_TOKEN` alone does not enable YouTube research. If `TUBELAB_API_KEY` is not set, this skill is unavailable; skip it (see `content-planner` for how it degrades to X/Instagram/TikTok-only when this key is missing).
