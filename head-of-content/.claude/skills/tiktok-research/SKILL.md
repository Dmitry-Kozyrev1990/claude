...
- `APIFY_TOKEN` environment variable or in `.env` (required)
- `GEMINI_API_KEY` environment variable or in `.env` (optional — enables AI video analysis; without it, fetch and outlier detection still run and video analysis is skipped)
- `apify-client` Python package (`google-genai` only needed if `GEMINI_API_KEY` is set)
...
if not os.environ.get('GEMINI_API_KEY'):
    print('Note: GEMINI_API_KEY not set — video analysis step will be skipped')
...
### 4. Analyze Top Videos with AI (optional — requires `GEMINI_API_KEY`)

Skip this step if `GEMINI_API_KEY` is not set; proceed straight to report generation using `outliers.json` only.
