...
- `APIFY_TOKEN` for X, Instagram, and TikTok research (required)
- `TUBELAB_API_KEY` for YouTube research (optional — if not set, skip the YouTube subagent and run X/Instagram/TikTok only)
- `GEMINI_API_KEY` for video analysis (optional — if not set, skip the video-analysis step in each subagent; outlier reports still generate)
- Accounts configured in `.claude/context/` for each platform

**Apify-only mode**: with only `APIFY_TOKEN` set, run steps 3 onward using just the X, Instagram, and TikTok subagents (skip Subagent 3 - YouTube Research entirely), and have each subagent skip its video-analysis step. Adjust step 4 onward to read from 3 platforms instead of 4, and generate 3 playbooks instead of 4.
...
Use the Task tool to launch 4 subagents simultaneously (or 3, skipping Subagent 3 - YouTube Research, when `TUBELAB_API_KEY` is not set — Apify-only mode):
...
**Subagent 3 - YouTube Research (skip in Apify-only mode, i.e. when `TUBELAB_API_KEY` is not set):**
```
...
After all subagents complete, read from each platform's latest run folder (`video-analysis.json` only exists when `GEMINI_API_KEY` was set; `youtube-research/` only exists when `TUBELAB_API_KEY` was set):
...
youtube-research/{latest}/ (skip if Apify-only mode)
├── outliers.json
└── video-analysis.json (if exists)
...
4. **Calculate opportunity scores** for X ideas:
   ...
   - `youtube_saturation`: same scale (omit this term in Apify-only mode, i.e. when YouTube research was skipped)
...
For each researched platform, read `references/playbook-template.md` and generate:

- `{RUN_FOLDER}/x-playbook.md`
- `{RUN_FOLDER}/instagram-playbook.md`
- `{RUN_FOLDER}/youtube-playbook.md` (omit in Apify-only mode)
- `{RUN_FOLDER}/tiktok-playbook.md`

Each playbook extracts from the platform's research:
- Winning hooks with replicable formulas (from video-analysis.json, when present)
...
└── {YYYY-MM-DD_HHMMSS}/
    ...
    ├── youtube-playbook.md       # YouTube intelligence playbook (omitted in Apify-only mode)
...
2. Launch 4 research subagents in parallel (Task tool with 4 invocations), or 3 in Apify-only mode (`TUBELAB_API_KEY` unset — skip YouTube)
3. Wait for all subagents to complete
4. Read all outliers.json and video-analysis.json files (video-analysis.json only where `GEMINI_API_KEY` was set)
5. Generate content-ideas.md using cross-platform analysis
6. Generate a playbook per researched platform (4, or 3 in Apify-only mode)
