...
- `APIFY_TOKEN` (required) - For X, Instagram, and TikTok scraping (get from https://console.apify.com/account/integrations)
- `TUBELAB_API_KEY` (optional) - For YouTube outlier detection (get from https://tubelab.net/settings/api). Without it, `youtube-research` is unavailable and `content-planner` runs in **Apify-only mode**: X, Instagram, and TikTok research only.
- `GEMINI_API_KEY` (optional) - For video analysis and thumbnail generation (get from https://aistudio.google.com/api-keys). Without it, each research skill still fetches content and detects outliers, but skips the AI video-analysis step (no hooks/structure breakdown in reports).

With only `APIFY_TOKEN` set, the toolkit is fully functional for X, Instagram, and TikTok research and content planning — reports just won't include AI-derived hook/structure analysis or YouTube data.
